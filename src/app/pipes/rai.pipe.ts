...
export class RaiPipe implements PipeTransform {
  ...
  mrai = 10000000000000000000000000000000000;
  krai = 10000000000000000000000000000000;
  rai  = 10000000000000000000000000000;
  ...
  transform(value: any, args?: any): any {
    ...
    switch (denomination.toLowerCase()) {
      default:
      case 'xrb': return `${(value / this.mrai).toFixed(6)}${!hideText ? ' ADIA' : ''}`;
      case 'mnano':
        ...
        if (hasRawValue) {
          ...
          return `${this.toFixed(newVal, this.precision)}${!hideText ? ' ADIA' : ''}`;
        } else {
          return `${(value / this.mrai).toFixed(6)}${!hideText ? ' ADIA' : ''}`;
        }
      case 'knano': return `${(value / this.krai).toFixed(3)}${!hideText ? ' knano' : ''}`;
      case 'nano': return `${(value / this.rai).toFixed(0)}${!hideText ? ' adia' : ''}`;
