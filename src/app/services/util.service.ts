...
function getPublicAccountID(accountPublicKeyBytes, prefix = 'adia') {
...
function isValidAccount(account: string): boolean {
  return pawcurrency.checkAddress(account);
}

// Check if a string is a numeric and larger than 0 but less than arcadia supply
function isValidNanoAmount(val: string) {
  // numerics and last character is not a dot and number of dots is 0 or 1
  const isnum = /^-?\d*\.?\d*$/.test(val);
  if (isnum && String(val).slice(-1) !== '.') {
    if (val !== '' && mnanoToRaw(val).gte(1) && pawcurrency.checkAmount(mnanoToRaw(val).toString(10))) {
      return true;
    } else {
      return false;
    }
...
function isValidAmount(val: string) {
  return pawcurrency.checkAmount(val);
}

function getAccountPublicKey(account) {
  if (!isValidAccount(account)) {
    throw new Error(`Invalid arcadia account`);
  }
  const account_crop = account.length === 64 ? account.substring(4, 64) : account.substring(5, 65);
  const isValid = /^[13456789abcdefghijkmnopqrstuwxyz]+$/.test(account_crop);
  if (!isValid) throw new Error(`Invalid arcadia account`);
...
function setPrefix(account, prefix = 'paw') {
  if (prefix === 'adia') {
    return account.replace('paw_', 'adia_');
  } else {
    return account.replace('adia_', 'paw_');
  }
}
 ...
 */
const mnano = 10000000000000000000000000000000000;
const knano = 10000000000000000000000000000000;
const nano  = 10000000000000000000000000000;
...
function isValidSeed(val: string) {
  return pawcurrency.checkSeed(val);
}

function isValidHash(val: string) {
  return pawcurrency.checkHash(val);
}

function isValidIndex(val: number) {
  return pawcurrency.checkIndex(val);
}

function isValidSignature(val: string) {
  return pawcurrency.checkSignature(val);
}

function isValidWork(val: string) {
  return pawcurrency.checkWork(val);
}

function validateWork(blockHash: string, threshold: string, work: string) {
  return pawcurrency.validateWork({blockHash: blockHash, threshold: threshold, work: work});
}
