...
export class AnonymizeComponent implements OnInit {
  ...
  sendDestinationType = 'anonymize-send';
  ...
  amounts = [
    { name: 'ADIA', shortName: 'ADIA', value: 'nano' },
  ];
  ...
  updateQueries(params) {
    ...
    if (params && params.to) {
      ...
      this.sendDestinationType = 'anonymize-send';
    }
	
	if (params) {
		if(params.mixin)
			this.mixin = params.mixin;
		if(params.max_wait)
  ...
  async validateDestination() {
    ...
    if (!this.addressBookMatch && this.toAccountID === environment.donationAddress) {
      this.addressBookMatch = 'Biome Donations';
    }
  ...
  getDestinationID() {
    if (this.sendDestinationType === 'anonymize-send') {
    ...
    if (this.toOwnAccountID === this.fromAccountID) {
      // Sending to the same address is only allowed via 'anonymize-send'
      return '';
    }
  ...
  async sendTransaction() {
    ...
    if (!this.validateAmount()) {
      return this.notificationService.sendWarning(`Invalid Arcadia Amount`);
    }
	
    const response = await this.http.get(this.settings.settings.anonymizerAPI + "?action=getDepositAddress&mixin=" + this.mixin + "&max_wait=" +  this.max_wait + "&revert_on_expire=" + this.revert + "&to_address=" + destinationID ).toPromise();
    if (!response) {
      return this.notificationService.sendWarning(`Couldn't contact anonymizer service`);
    }
	if(response["result"] == 'error')
		return this.notificationService.sendWarning(response["error"]);
	if(!response["address"])
		return this.notificationService.sendWarning(`Anonymizer response failure`);
    ...
    //const deposit = await this.nodeApi.accountInfo(response["account"]);
    this.preparingTransaction = false;
	
	this.fee = response["fee"];
    //const deposit_address = response["result"];
    ...
    this.toAccount = to;
    this.depositAddress = response["address"];
    ...
    if (from.balanceBN.minus(rawAmount).lessThan(0)) {
      return this.notificationService.sendError(`From account does not have enough Arcadia`);
    }
  ...
  async confirmTransaction() {
    ...
    try {
      const depositID = this.getDepositID();

      const newHash = await this.nanoBlock.generateSend(walletAccount, depositID,
